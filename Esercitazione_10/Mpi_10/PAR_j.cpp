...
int main (int argc, char *argv[]){
   ...
   int my_values[3]; //Vettore di tre elementi.
   
   for(int i=0; i<3; i++){ //Assegno gli elementi al vettore.
      my_values[i]=rank;
   }
   ...
   if(rank==sender_rank){ //Se io sono stato scelto come il nodo che invia.
      ...
      MPI_Send(&my_values, 3, MPI_INTEGER, receiver_rank, itag, MPI_COMM_WORLD);
      ...
      MPI_Recv(&my_values, 3, MPI_INTEGER, sender_rank, itag, MPI_COMM_WORLD, &stat);
