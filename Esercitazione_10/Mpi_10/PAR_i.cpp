...
int main (int argc, char *argv[]){
   ...
   //for(int i=0; i<2; i++){ //Ripeto piu' volte il processo di scambio.
   ...
   int imesg = rank; //Il messaggio da inviare e' il proprio numero di nodo.

   cout << "Prima. Sono il nodo " << rank << " e il mio messaggio e' " << imesg << endl;
   ...
   if(rank==sender_rank){ //Se io sono stato scelto come il nodo che invia.
      ...
      MPI_Send(&imesg, 1, MPI_INTEGER, receiver_rank, itag, MPI_COMM_WORLD);

      cout << "Sono il nodo " << rank << " e ho INVIATO il messaggio a " << receiver_rank << endl;  
   }
      ...
      MPI_Recv(&imesg, 1, MPI_INTEGER, sender_rank, itag, MPI_COMM_WORLD, &stat);

      cout << "Sono il nodo " << rank << " e ho RICEVUTO il messaggio da " << sender_rank << endl;   
   }
   ...
   cout << "Dopo. Sono il nodo " << rank << " e il mio messaggio e' " << imesg << endl;
  ...
  /*Esempio di output per 4 nodi.
   ...
   Prima. Sono il nodo 0 e il mio messaggio e' 0 <-----
   Sono il nodo 0 e sono receiver rank!
   Prima. Sono il nodo 1 e il mio messaggio e' 1
   Sono il nodo 1 e non ho nessun compito.
   Dopo. Sono il nodo 1 e il mio messaggio e' 1
   Prima. Sono il nodo 2 e il mio messaggio e' 2
   Sono il nodo 2 e non ho nessun compito.
   Dopo. Sono il nodo 2 e il mio messaggio e' 2
   Prima. Sono il nodo 3 e il mio messaggio e' 3
   Sono il nodo 3 e sono sender rank!
   Sono il nodo 3 e ho INVIATO il messaggio a 0
   Dopo. Sono il nodo 3 e il mio messaggio e' 3
   Sono il nodo 0 e ho RICEVUTO il messaggio da 3
   Dopo. Sono il nodo 0 e il mio messaggio e' 3 <-----
   */
