...
int main (int argc, char *argv[]){
   ...
   //cout << "Sono il nodo " << rank << " e ho estratto il numero " << caso << endl;  
   ...
   MPI_Reduce(&mesg, &irecv, 1, MPI_INTEGER, MPI_MAX, 0, MPI_COMM_WORLD); 

   //Stampa i risultati solo il nodo = 0.
   if(rank==0){
      cout << "Massimo dei rank: " << irecv << endl;   
   } 
   ...
   cout << "Sono " << rank << " e ho ricevuto il massimo di tutti = " << irecv << endl;
